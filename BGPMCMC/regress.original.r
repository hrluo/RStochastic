...
#
# Data:   y(xi) = F*beta+eta(xi), i=1,...,n ; xi=1xK vector
#
#
# Likelihood: L(Y|.) propto. 1/(2pi*det(E))exp(-(Y-F*beta)'(Einv)(Y-F*beta)) ; Y=[y(x1),...,y(xn)]
# where,
#         E=1/lambdaz*[R(.,.;rho)]
#
# Priors:
...
#         pi(betaz) propto. 1/(2pi*deta(lambdamu*Ip) )exp(-(betaz-mu)'(posterior Rinv)(betaz-mu)) [normal prior]
#
# Posterior:
#         pi(.|Y) propto. L(Y|.)pi(lambdaz)pi(rhoz_1)*...*pi(rhoz_K)*pi(betaz)
#
...
# mh=list(rr=.07)
regress<-function(y,l.dez,N,pi,mh,last=1000,adapt=TRUE,cheat=1e-14)
{
...
plotfit2<-function(fit,simtrue=NULL){
  par(mfrow=c(2,4))
  plot(fit$lambdaz,type='l',xlab="draw",ylab="lambdaz")
  #abline(h=simtrue$lambdaytrue,col="red")
  plot(fit$rhoz[,1],type='l',xlab="draw",ylab="rhoz_1")
  #abline(h=simtrue$rhotrue,col="red")
  plot(fit$rhoz[,2],type='l',xlab="draw",ylab="rhoz_2")
  #abline(h=simtrue$rhotrue,col="red")
  plot(fit$betaz,type='l',xlab="draw",ylab="betaz")
  #abline(h=simtrue$betatrue,col="red")
  ...
  boxplot(fit$lambdaz,type='l',xlab="draw",ylab="lambdaz")
  #abline(h=simtrue$lambdaytrue,col="red")
  boxplot(fit$rhoz[,1],type='l',xlab="draw",ylab="rhoz_1")
  #abline(h=simtrue$rhotrue,col="red")
  boxplot(fit$rhoz[,2],type='l',xlab="draw",ylab="rhoz_2")
  #abline(h=simtrue$rhotrue,col="red")
  boxplot(fit$betaz,type='l',xlab="draw",ylab="betaz")
  #abline(h=simtrue$betatrue,col="red")
...
predict<-function(l.v,fit,eps.yy=0,eps=1e-6,force=F)
{
  ...
  for(i in 1:N)
  {
    ...
    #Ryy.inv=chol2inv(chol(Ryy+diag(n)*eps.yy,pivot=force))
    Ryy.inv=ginv(Ryy+diag(n)*eps.yy)
    ...
    L=t(svd(E.cond+diag(m)*eps)$v)
    #We use ginv to avoid the problem of close to singularity matrix inversion. However, this will lead to overly smoothness at some locations.
