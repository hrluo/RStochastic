...
save(results, file = "simulation_effect_draws_1_SecondPart.Rdata")
...
load(file = "simulation_effect_draws_1.Rdata")
load(file = "C:/Users/Giovanni/Google Drive/PhD/Classes/STAT 8810/final project/results/Result effect draws - 1/simulation_effect_draws_1.Rdata")
...
for(fitString in names(results[[1]])) {
  ...
  pdf(paste0(fitString, "_1.pdf"), onefile = T)
  
  for(i in 1:length(results)) {
      resultsIter <- results[[i]]
      plotGpPseudoInputs(resultsIter[[fitString]], cond = 1e-8)
      mseResults[i,fitString] <- mse(resultsIter[[fitString]], cond = 1e-8)
  }
...
boxplot(log(mseResults$fit_givenPI_grid, base = 10), 
        ...
        main = "Prediction error - Smooth functions") #,ylim = c(0, .005)


dev.copy(pdf, file = "summary_effect_draws_1.pdf", height = 6, width = 8)
dev.off()
