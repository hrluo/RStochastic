...
save(results, file = "simulation_effect_m_1.Rdata")
...
load(file = "simulation_effect_m_1.Rdata")
...
plot(mseResults$m, log(mseResults$fit_givenPI_grid, base = 10), 
     main ="Prediction error - Smooth function", 
     type = "b",  lwd =2, 
     ylab = expression('log'[10]*'(Prediction error)'), 
     xlab = "Number of pseudo-inputs", 
...
dev.copy(pdf, file = "summary_effect_m_1.pdf", height = 6, width = 8)
dev.off()
